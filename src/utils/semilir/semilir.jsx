...
const Similar = () => {
  ...
  const fetchSemilar = (imdbId) => {
    const semilarURL = `https://api.themoviedb.org/3/tv/${imdbId}/similar?api_key=cf26269a358b763332b2e7d79ba63ff3`
    fetch(semilarURL)
    .then(resp => resp.json())
    .then(data => {
  ...
  <Swiper
    ...
    {isLoading ? <Skeleton variant="rounded" animation="wave" width={100} height={190} style={{borderRadius: '5px', marginLeft:'5px'}}/> :
      <Link to={`${item.id}`} className='cardMoviesimilar' key={item.id}>
